  ...
  <v-expansion-panels v-model="timer_panel" focusable>
    <v-expansion-panel v-for="(timer, index) in cfg.timers" :key="index" class="grey darken-1">
      ...
      <v-expansion-panel-content>
        ...
        <v-text-field type="text" :value="timer.time" @change="v => timer.time = v" :label="$t('timers.time_lbl')" :rules="[rules.required]"></v-text-field>
        ...
        <v-btn outlined color="orange darken-1" @click="timer_confirm_idx= index; confirm_dialog= true;"><v-icon>mdi-delete</v-icon></v-btn>
      </v-expansion-panel-content>
    </v-expansion-panel>
  </v-expansion-panels>
